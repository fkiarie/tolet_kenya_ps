...
// fetch ledger row to get tenant/unit/landlord
...
// find landlord via building
$stmt2 = $conn->prepare("SELECT b.landlord_id FROM units u JOIN buildings b ON u.building_id = b.id WHERE u.id = ?");
stmt2:
$stmt2->execute([$unit_id]);
$landlord_id = $stmt2->fetchColumn();
if (!$landlord_id) die("Landlord not found");

// agent is current logged-in agent
$agent_id = current_agent_id();
if (!$agent_id) die("Agent not in session");

// store payment and then recalc ledger in transaction
try {
    $conn->beginTransaction();
    $ins = $conn->prepare("INSERT INTO payments (ledger_id, unit_id, tenant_id, landlord_id, agent_id, amount_paid, payment_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $ins->execute([$ledger_id, $unit_id, $tenant_id, $landlord_id, $agent_id, $amount_paid, $payment_date]);

    // recalc totals for the ledger
